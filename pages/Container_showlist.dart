...
class _ContainerShowListPageState extends State<ContainerShowListPage> {
  ...
  Future<void> getData() async {
    final response = await http.get(Uri.parse(
        'http://10.0.2.2/code_team4/public/Flutter_container/get_all/1'));
                ...
                child: ListTile(
                  leading: CircleAvatar(
                      backgroundImage: NetworkImage(
                          'https://packer.searates.com/images/main-container.png')),
                  title: Text('${container[index]['con_number']}',
                      style:
                          const TextStyle(fontSize: 20, color: Colors.black)),
                  subtitle: Text('${container[index]['cont_name']}',
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () {
            Navigator.push(context,
                    MaterialPageRoute(builder: (context) => AddContainerPage()))
                .then((value) async {
              await getData();
            });
          },
